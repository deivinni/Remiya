...
module.exports = {
    run: async(msg) => {
        ...
        .setTimestamp();
        let get_image;
        if (msg.args[0] == '--gif') get_image = msg.config.get_images.nekos.nsfw.nekoGif;
        if (msg.args[0] == '--ero') get_image = msg.config.get_images.nekos.nsfw.eroNeko
        if (!msg.args[0])           get_image = msg.config.get_images.nekos.nsfw.neko;
