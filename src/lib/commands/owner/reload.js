...
module.exports = {
  run: async(msg) => {
    ...
    if (!path) return msg.channel.send(`${msg.config.e_men.pasta} \`|\` ${msg.author}, estas são as pastas de comandos: ${readdirSync('./src/lib/commands').map(a => `\`${a}\``).join(', ')}`);
    if (!cmd) return msg.channel.send(`${msg.config.e_men.cmd} \`|\` ${msg.author}, estes são os comandos da pasta **${path}**: ${readdirSync(`./src/lib/commands/${path}`).map(a => `\`${a}\``.replace('.js','')).join(', ')}`);
    try {
