...
module.exports = {
    run: async(msg) => {
        try {
            if (!msg.args[0]) return msg.channel.send(`${msg.config.e_men.errado} \`|\` ${msg.author}, você deve colocar algo para nas documentações do discord.js.`);
            request({
                url: `https://djsdocs.sorta.moe/main/stable/embed?q=${encodeURIComponent(msg.args.join(' '))}`, 
                json: true
            }, (req, res, json) => {
                    if (!json) return msg.channel.send(`${msg.config.e_men.errado} \`|\` ${msg.author}, não consegui encontrar nada.`);
